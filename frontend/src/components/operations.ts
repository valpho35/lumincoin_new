...
export class Operations {
    ...
    private operationToDelete: string | null = null;
    ...
    async init() {
        (window as any).operationsInstance = this;
        await this.loadOperations();
        this.setUpEvents();
        MenuUtils.initOperationsMenu();
    }

    async loadOperations(period?: string) {
         try {
            let url = '/operations';
            ...
            const response = await ApiUtils.request('GET', url);
    ...
    displayOperations(operations: any[]) {
    ...
    showDeletePopup(operationId: string) {
